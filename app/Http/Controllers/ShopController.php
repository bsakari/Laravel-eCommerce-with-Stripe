...
class ShopController extends Controller {
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function category(Request $request, $uri, $category)
    {
        $order = !empty($request->get('order')) ? $request->get('order') : 'asc';

        $perPage = 12;

        // Retrieve a list of this Category and all Sub-Categories
        $allCategories = CategoryHelper::getCategoryAndChildrenOfCategory(app()->global['categories']['all']->toArray(), $category['id']);

        // Retrieve parent Category
        $parentCategory = $category['parent_id'] > 0 ? Category::find($category['parent_id']) : null;

        // Retrieve Products in this Category and all Sub-Categories
        $products = Product::select('products.*')
            ->whereIn('category_id', collect($allCategories)->pluck('id')->toArray());
        ...
        return view('shop.categories.show', compact('category', 'parentCategory', 'products'));
    }
