...
class AttributesController extends Controller {
    ...
    public function store(Request $request)
    {
        ...
        if ($createdAttribute) {
            return redirect(route('admin.attributes.show', $createdAttribute['id']))->with('alert-success', 'The attribute has been added successfully.');
        } else {
    ...
    public function update(Request $request, $attribute)
    {
        ...
        if ($result) {
            return redirect(route('admin.attributes.show', $attribute['id']))->with('alert-success', 'The attribute has been updated successfully.');
        } else {
    ...
    public function destroy($attribute)
    {
        if ($attribute->options() && $attribute->options()->count() > 0) {
            ...
            return redirect(route('admin.attributes.index'))->with('alert-success', 'The attribute has been removed successfully.');
        }
    }
