@component('layouts.admin')
    ...
    @slot('title')
        Admin - Order #{{ $order['order_number'] }} - {{ config('app.name') }}
    @endslot
    ...
    <div class="row content">
        ...
        <div class="col-md-12">
            ...
            @if ($order->coupon()->count() > 0)
                Coupon used: <a href="{{ route('admin.coupons.show', $order->coupon['id']) }}" title="View the Coupon"><span class="label label-primary">{{ $order->coupon['code'] }}</span></a><br /><br />
            @endif
                    ...
                    @foreach ($order->inventoryItems as $item)
                        <tr>
                            <td class="image">
                                ...
                                <a href="{{ route('shop.product', [$item->product['uri'], $item->product['id']]) }}" title="View this item">{{ $item->product['name'] }}</a>
            ...
            <div class="basket-total">
                ...
                <p class="text-right"><span class="pull-left">Shipping:</span> ${{ number_format($order['shipping_fee'], 2) }} USD</p>
                @if (!empty($order['discount']) && $order['discount'] > 0)
                    <p class="text-right"><span class="pull-left">Discount:</span> - ${{ number_format($order['discount'], 2) }} USD</p>
                @endif
                <hr />
